...
void Matcher::matchFilesToAlbums(const vector<Metafile *> &files) {
	...
	if (only_save_if_all_match && total_matched != files.size())
		return;
	/* set new metadata */
}

bool Matcher::saveMatchToCache(const string &filename, const string &track_mbid, double score) const {
	...
	query.str("");
	query << "UPDATE match SET score = " << score << " WHERE file_id = (SELECT file_id FROM file WHERE filename = '" << e_filename << "') AND metatrack_id = (SELECT metatrack_id FROM metatrack WHERE track_mbid = '" << e_track_mbid << "')";
	if (!locutus->database->query(query.str()))
		locutus->debug(DEBUG_NOTICE, "Unable to save metadata match in cache, query failed. See error above");
	return true;
}
