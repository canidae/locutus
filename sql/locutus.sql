...
CREATE VIEW v_web_album_matching_list_albums AS
    SELECT a.album_id, a.title AS album, count(DISTINCT t.track_id) AS tracks, count(DISTINCT m.track_id) AS tracks_matched, count(DISTINCT m.file_id) AS files_matched, sum((m.mbid_match)::integer) AS mbids_matched, max(m.meta_score) AS max_score, min(m.meta_score) AS min_score, avg(m.meta_score) AS avg_score, (sum(m.meta_score) / (count(DISTINCT t.track_id))::double precision) AS album_score FROM ((album a JOIN track t ON ((t.album_id = a.album_id))) LEFT JOIN (SELECT DISTINCT ON (t.album_id, m.file_id) m.track_id, m.file_id, m.mbid_match, m.meta_score FROM (match m JOIN track t ON ((t.track_id = m.track_id))) ORDER BY t.album_id, m.file_id, m.mbid_match DESC, m.meta_score DESC) m ON ((m.track_id = t.track_id))) WHERE (a.album_id IN (SELECT t.album_id FROM ((match m JOIN file f ON ((f.file_id = m.file_id))) JOIN track t ON ((t.track_id = m.track_id))) WHERE (f.matched IS NULL))) GROUP BY a.album_id, a.title;
