...
$vars{'albums'} = $dbh->selectall_arrayref('SELECT * FROM v_web_list_album_matching WHERE matched > 0 ORDER BY matched DESC, mbid_matches DESC, tracks ASC, avg_score ASC', {Slice => {}});
