...
foreach my $track (@{$vars{tracks}}) {
	$track->{matches} = $dbh->selectall_arrayref('SELECT * FROM v_web_track_list_matching_files WHERE track_id = ' . $dbh->quote($track->{track_id}) . ' ORDER BY mbid_match DESC, puid_match DESC, meta_score DESC LIMIT 1', {Slice => {}});
