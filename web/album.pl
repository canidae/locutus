...
$vars{tracks} = $dbh->selectall_arrayref('SELECT * FROM (SELECT DISTINCT ON (album_id, file_id) * FROM v_web_album_list_tracks_and_matching_files WHERE album_id = ' . $alid . ' ORDER BY album_id, file_id, mbid_match desc, meta_score desc) tmp ORDER BY tracknumber ASC, mbid_match DESC, meta_score DESC', {Slice => {}});
